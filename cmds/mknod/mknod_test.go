...
func TestInvocationErrors(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{
			args: []string{devpath},
			want: "Usage: mknod path type [major minor]\n",
		},
		{
			args: []string{""},
			want: "Usage: mknod path type [major minor]\n",
		...
		{
			args: []string{devpath, "p", "254", "3"},
			want: "device type p requires no other arguments\n",
		},
		{
			args: []string{devpath, "b", "254"},
			want: "Usage: mknod path type [major minor]\n",
		},
		{
			args: []string{devpath, "b"},
			want: "device type b requires a major and minor number\n",
		},
		{
			args: []string{devpath, "k"},
			want: "device type not recognized: k\n",
		},
	} {
